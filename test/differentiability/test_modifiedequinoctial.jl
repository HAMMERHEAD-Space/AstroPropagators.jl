@testset "ModEq State Differentiability" begin
    SpaceIndices.init()
    for backend in _BACKENDS
        testname = "ModEq Differentiability " * backend[1]
        @testset "$testname" begin
            f_fd, df_fd = value_and_jacobian(
                (x) -> ModEq_EOM(x, _p2, _t, _model_list),
                AutoFiniteDiff(),
                Array(_state_modeq),
            )

            f_ad, df_ad = value_and_jacobian(
                (x) -> Array(ModEq_EOM(x, _p2, _t, _model_list)),
                backend[2],
                Array(_state_modeq),
            )
...
@testset "ModEq Time Differentiability" begin
    SpaceIndices.init()
    for backend in _BACKENDS
        testname = "ModEq Differentiability " * backend[1]
        @testset "$testname" begin
            f_fd, df_fd = value_and_derivative(
                (x) -> ModEq_EOM(Array(_state_modeq), _p2, x, _model_list),
                AutoFiniteDiff(),
                _t,
            )

            f_ad, df_ad = value_and_derivative(
                (x) -> Array(ModEq_EOM(Array(_state_modeq), _p2, x, _model_list)),
                backend[2],
                _t,
            )
...
@testset "ModEq Parameter Differentiability" begin
    SpaceIndices.init()
    ...
    function dynamics_params(x::AbstractArray{T}) where {T<:Number}
        ...
        return ModEq_EOM(_state_modeq, _p2, _t, model_list)
    end
