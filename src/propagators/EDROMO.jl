...
export EDromo_end_condition, end_EDromo_integration
...
function EDromo_EOM(
    ...
    ##################################################
    u_cart = Cartesian(EDromo(u), p.μ; DU=DU, TU=TU, W=W, ϕ=ϕ, t₀=t₀, flag_time=flag_time)
    tt = get_EDromo_time(u; DU=DU, TU=TU, W=W, t₀=t₀, ϕ=ϕ, flag_time=flag_time)

    ##################################################
    #* 3. Potential Based Perturbations
    ##################################################
    kep_model = KeplerianGravityAstroModel(μ=p.μ)
    U = potential(u_cart, p, tt, models.gravity_model) - potential(u_cart, p, tt, kep_model)
    ∇Uᵣ_inertial = acceleration(u_cart, p, tt, models.gravity_model) - acceleration(u_cart, p, tt, kep_model)
    ∇Uₜ = potential_time_derivative(u_cart, p, tt, models.gravity_model) - potential_time_derivative(u_cart, p, tt, kep_model)
    ...
    ##################################################
    p_inertial = AstroForceModels.sum_accelerations(u_cart, p, tt, models.perturbing_models)
...
@inline function inert2orb_edromo(
    ...
    R1_T = 2.0 * SMatrix{3,3}(
        0.5-ζ5^2-ζ6^2, ζ4*ζ5-ζ6*ζ7, ζ4*ζ6+ζ5*ζ7,
        ζ4*ζ5+ζ6*ζ7,  0.5-ζ4^2-ζ6^2, ζ5*ζ6-ζ4*ζ7,
        ζ4*ζ6-ζ5*ζ7,  ζ5*ζ6+ζ4*ζ7,  0.5-ζ4^2-ζ5^2
    )
...
function EDromo_end_condition(
    u::AbstractVector, ϕ::Number, integrator::T; DU::Number, TU::Number, W::Number, t₀::Number, flag_time::AstroCoords.AbstractTimeType, stop_time::Number=0.0
...
end_EDromo_integration(stop_time::Number; DU::Number, TU::Number, W::Number, t₀::Number, ϕ::Number, flag_time::AstroCoords.AbstractTimeType) = ContinuousCallback(
    (u, ϕ, integrator) -> EDromo_end_condition(
        u, 
        ϕ, 
        integrator; 
        DU=DU, 
